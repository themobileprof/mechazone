...
import { closeoutSession, decodeVin, fetchHistory, identity, ingestSession, ledgerOnline, lookupDtc } from './api'
import { enqueue, flushQueue, pendingCount } from './queue'
import type { HistoryResponse, ScanResult, Session } from './types'
        ...
        <section className="rounded-sm border border-brass/20 bg-panel p-5">
          ...
          <button className="mb-4 min-h-12 w-full border border-brass px-4 text-brass" disabled={!scan} onClick={() => run('log', async () => {
            ...
            const body = {
              ...
              technician_id: identity.technicianId,
          ...
          <p className="mt-3 font-mono text-[11px] text-steel">Playbook AI is Phase 2. History and verified fixes already rank first.</p>
        </section>
      </div>
    </div>
  )
