...
ok "This sets up the bay app on this computer. You only do this once."
...
(
  ...
  GOTOOLCHAIN=local go build -o "$ROOT/bin/mechazone-server" ./cmd/server
)
ok "Ledger ready"
...
if [[ -d "$ROOT/third_party/j2534/j2534" ]] && command -v pkg-config >/dev/null && pkg-config --exists libusb-1.0 2>/dev/null; then
  ...
  if [[ -f "$ROOT/third_party/j2534/j2534/j2534.so" ]]; then
    if grep -q '^J2534_LIB=' .env; then
      sed -i "s|^J2534_LIB=.*|J2534_LIB=$ROOT/third_party/j2534/j2534/j2534.so|" .env
    else
      printf 'J2534_LIB=%s/third_party/j2534/j2534/j2534.so\n' "$ROOT" >> .env
    fi
    ok "OpenPort library built"
  fi
fi
